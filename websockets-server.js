...
ws.on("connection", function(socket) {
  ...
  socket.on("message", function(data) {
    ...
    if (topicFlag == 0) { //the tag '/topic' was found as the first item in the string
      //message = []; //reset array Ver A
      //message.push("*** Topic is '" + data.substring(7) + "' ***"); //Push this message into the array Ver A
      message.unshift("*** Topic is '" + data.substring(7) + "' ***"); //incase the prof wanted ALL the chat since the chat first began Ver B
